...
CREATE TABLE `users`(
  ...
  `oid` int(11) UNSIGNED NOT NULL COMMENT '关联职业表主键id',
  ...
  `label` varchar(255) NOT NULL COMMENT '个性标签（序列化数组）',
...
# 城市表
CREATE TABLE `city`(
  `id` int(11) UNSIGNED NOT NULL AUTO_INCREMENT COMMENT '城市表主键id',
...
CREATE TABLE `service`(
  ...
  `uid` int(11) UNSIGNED NOT NULL COMMENT '关联用户信息表主键id',
  `scid` int(11) UNSIGNED NOT NULL COMMENT '关联服务表主键id',
  `cover_path` varchar(255) NOT NULL COMMENT '封面路径',
  `describe` varchar(5000) NOT NULL COMMENT '描述',
  `gameid` varchar(50) DEFAULT '' COMMENT '游戏ID',
  `region` varchar(50) DEFAULT '' COMMENT '所属大区',
  `dan_grading` varchar(50) DEFAULT '' COMMENT '当前段位',
  `voice` varchar(255) DEFAULT '' COMMENT '语音路径',
  `price` decimal(14,2) UNSIGNED NOT NULL COMMENT '价格',
  `or_quantity` int(10) UNSIGNED NOT NULL COMMENT '接单数',
  `or_start_time` varchar(10) NOT NULL COMMENT '接单开始时间',
  `or_end_time` varchar(10) NOT NULL COMMENT '接单结束时间',
